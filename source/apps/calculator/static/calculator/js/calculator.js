...
let createCurrProd = function() {
    ...
    prod_select.addEventListener('change', function choose_product() {
        ...
        let product_name;
        // можно было через бинарный поиск; не стал его реализовывать в js...
...
async function fetchHandler(url) {
    ...
    let default_opt = setDefaultOpt("Выберите продукт")
    prod_select.append(default_opt);
    console.log(data)
...
cat_select.addEventListener('change', function select_cat() {
    ...
    if (category_id) {
        ...
        fetchHandler(api_url); // запрос отправляется и на базе ответа формируется дропдаун с продуктами...
        // глобальный current принимает ссылку на объект...
        current = createCurrProd();  // начинается прослушивание дропдауна и ввода количества продукта и на основе выбранного продукта и количества
    };
});
    ...
    if (xhr.readyState === 4) { 
        if (xhr.status === 200) {
            ...
            xhr.onload = function() {
                ...
                html.innerHTML = xhr.response;  // заменяем html полученным от Django...
              };
        } else {
            alert(xhr.status)
        }
